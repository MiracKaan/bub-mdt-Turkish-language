      ...
      <div className='left-side'>
        <div className='header'>
          ...
          <Text color="dimmed" size="xs" style={{display: 'flex', gap: 10}}>
            <IconCalendar size={rem(18)} /> <Text color="dimmed" size={14}>{new Date().toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</Text>
          </Text>
        </div>
                  ...
                  {activeOfficers.map((officer) => (
                    <div className='active-officer-card'>
                      <Group style={{ width: '100%' }}>
                        ...
                        <Text color='dimmed' size='xs'>
                          {locales.attached_unit}: {officer.unitId || 'Yok'}
                        </Text>
                      </Group>
                    </div>
                  ))}
