...
const RecentActivity = () => {
  ...
  const rows = recentActivity.map((item, index) => (
    <tr key={index}>
      <td>
        <Badge color={item.type === 'created' ? "green" : (item.type === 'updated' ? 'yellow' : 'red')} radius="sm">
          {item.type === 'created' ? 'Oluşturuldu' : (item.type === 'updated' ? 'Güncellendi' : 'Silindi')}
        </Badge>
      </td>

      <td>
        <Text>{item.category === 'profiles' ? 'Profiller' : (item.category === 'incidents' ? 'Olaylar' : (item.category === 'reports' ? 'Raporlar' : (item.category === 'warrants' ? 'Aranmalar' : capitalizeFirstLetter(item.category))))}</Text>
      </td>
      <td>
      ...
      <td>
        {item.type !== 'deleted' && (
          <Group spacing={0} position="right">
            <Tooltip label="Görüntüle" color='gray' position="bottom" withArrow>
              <ActionIcon 
                onClick={async () => {
